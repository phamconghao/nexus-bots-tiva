...
void cmdTwistSpeedCallback(const geometry_msgs::Twist& cmd_twistSpeed)
{
    ...
    if (positive_inRange(0, 0.02, cmd_twistSpeed.linear.x))
    {
        mappedSpeed = geoLinear2mmps(0.02);
        omniNexusBot.setCarAdvance(mappedSpeed);
    }
    if (positive_inRange(0.02, 0.04, cmd_twistSpeed.linear.x))
    {
        mappedSpeed = geoLinear2mmps(0.04);
        omniNexusBot.setCarAdvance(mappedSpeed);
    }
    if (positive_inRange(0.04, 0.06, cmd_twistSpeed.linear.x))
    {
        mappedSpeed = geoLinear2mmps(0.06);
        omniNexusBot.setCarAdvance(mappedSpeed);
    }
    if (positive_inRange(0.06, 0.08, cmd_twistSpeed.linear.x))
    {
        mappedSpeed = geoLinear2mmps(0.08);
        omniNexusBot.setCarAdvance(mappedSpeed);
    }
    if (positive_inRange(0.08, 0.1, cmd_twistSpeed.linear.x))
    {
    ...
    if (negative_inRange(-0.02, 0, cmd_twistSpeed.linear.x))
    {
        mappedSpeed = geoLinear2mmps(0.02);
        omniNexusBot.setCarBackoff(mappedSpeed);
    }
    if (negative_inRange(-0.04, -0.02, cmd_twistSpeed.linear.x))
    {
        mappedSpeed = geoLinear2mmps(0.04);
        omniNexusBot.setCarBackoff(mappedSpeed);
    }
    if (negative_inRange(-0.06, -0.04, cmd_twistSpeed.linear.x))
    {
        mappedSpeed = geoLinear2mmps(0.06);
        omniNexusBot.setCarBackoff(mappedSpeed);
    }
    if (negative_inRange(-0.08, -0.06, cmd_twistSpeed.linear.x))
    {
        mappedSpeed = geoLinear2mmps(0.08);
        omniNexusBot.setCarBackoff(mappedSpeed);
    }
    if (negative_inRange(-0.1, -0.08, cmd_twistSpeed.linear.x))
    {
        mappedSpeed = geoLinear2mmps(0.1);
        omniNexusBot.setCarBackoff(mappedSpeed);
    }
    ...
    if (positive_inRange(0, 0.1, cmd_twistSpeed.angular.z))
    {
        mappedSpeed = geoAngular2mmps(0.1);
        omniNexusBot.setCarRotateLeft(mappedSpeed);
    }
    if (positive_inRange(0.1, 0.2, cmd_twistSpeed.angular.z))
    {
        mappedSpeed = geoAngular2mmps(0.2);
        omniNexusBot.setCarRotateLeft(mappedSpeed);
    }
    if (positive_inRange(0.2, 0.3, cmd_twistSpeed.angular.z))
    {
        mappedSpeed = geoAngular2mmps(0.3);
        omniNexusBot.setCarRotateLeft(mappedSpeed);
    }
    if (positive_inRange(0.3, 0.4, cmd_twistSpeed.angular.z))
    {
        mappedSpeed = geoAngular2mmps(0.4);
        omniNexusBot.setCarRotateLeft(mappedSpeed);
    }
    if (positive_inRange(0.4, 0.5, cmd_twistSpeed.angular.z))
    {
    ...
    if (negative_inRange(-0.1, 0, cmd_twistSpeed.angular.z))
    {
        mappedSpeed = geoAngular2mmps(0.1);
        omniNexusBot.setCarRotateRight(mappedSpeed);
    }
    if (negative_inRange(-0.2, -0.1, cmd_twistSpeed.angular.z))
    {
        mappedSpeed = geoAngular2mmps(0.2);
        omniNexusBot.setCarRotateRight(mappedSpeed);
    }
    if (negative_inRange(-0.3, -0.2, cmd_twistSpeed.angular.z))
    {
        mappedSpeed = geoAngular2mmps(0.3);
        omniNexusBot.setCarRotateRight(mappedSpeed);
    }
    if (negative_inRange(-0.4, -0.3, cmd_twistSpeed.angular.z))
    {
        mappedSpeed = geoAngular2mmps(0.4);
        omniNexusBot.setCarRotateRight(mappedSpeed);
    }
    if (negative_inRange(-0.5, -0.4, cmd_twistSpeed.angular.z))
    {
        mappedSpeed = geoAngular2mmps(0.5);
        omniNexusBot.setCarRotateRight(mappedSpeed);
    }
